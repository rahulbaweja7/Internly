...
export function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Account created! Welcome to Applycation.');
      navigate('/dashboard');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 flex items-center justify-center p-4 animate-fade-in">
      <div className="relative z-10 w-full max-w-md animate-slide-in-up">
        <div className="w-full rounded-2xl p-[1px] bg-gradient-to-r from-rose-400/40 via-amber-400/40 to-sky-400/40">
        <Card className="bg-white/95 dark:bg-[#0f1424]/95 border border-gray-200 dark:border-white/10 shadow-2xl rounded-2xl">
          ...
          <CardContent className="space-y-6">
            {/* Google Sign In — primary action first */}
            <Button
              type="button"
              onClick={() => { setIsLoading(true); window.location.href = `${config.API_BASE_URL}/api/auth/google`; }}
              disabled={isLoading}
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-white dark:bg-[#0f1424] px-2 text-gray-500 dark:text-gray-400">Or continue with email</span>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" className="w-full bg-gradient-to-r from-rose-500 to-amber-500 text-white hover:from-rose-400 hover:to-amber-400 transition-colors" disabled={isLoading}>
                {isLoading ? (
      ...
      <style>{`
        ...
        .animate-slide-in-up { animation: slide-in-up 0.6s ease-out 0.1s both; }
      `}</style>
    </div>
  );
}
